...
#             log_entry += "-" * 40 + "\n"
#     else:
#         log_entry += f"File: {results}\n"
#         log_entry += f"Response:\n{response}\n"
#     log_entry += "=" * 80 + "\n\n"

#     log_file_path = "general_log.txt"
...
#         code = f.read()
#         elements, _ = extract_ast_info(code)
#         ast_info = json.dumps(elements, indent=2)
#         log_query_results("AST", file_path, ast_info)
#         return ast_info
...
#         log_query_results("Suggest Changes", f"{file_path}\nPrompt: {prompt}", suggestion)
#         return suggestion
...
#                     # Log the query, relevant files, and response
...
def log_general_results(command, file_path, result):
...
def get_ast(file_path):
    with open(file_path, 'r') as f:
        ...
        elements, _ = extract_ast_info(code)
        ast_info = json.dumps(elements, indent=2)
        log_general_results("AST", file_path, ast_info)
        return ast_info
...
def suggest_changes(file_path, prompt):
    with open(file_path, 'r') as f:
        ...
        suggestion = response['choices'][0]['message']['content'].strip()
        log_general_results("Suggest Changes", f"{file_path}\nPrompt: {prompt}", suggestion)
        return suggestion
